...
class Setting extends MX_Controller {
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('currency_symbol',display('currency_symbol'),'required|max_length[5]'); 
		$this->form_validation->set_rules('currency_in_words',display('currency_in_words'),'required|max_length[50]');
		...
		$data['setting'] = (object)$postData = [
			...
			'currency_in_words'    => $this->input->post('currency_in_words', true),

			'soc_sec_npf_tax'      => $this->input->post('soc_sec_npf_tax', true),
			'employer_contribution'=> $this->input->post('employer_contribution', true),
			'icf_amount'           => $this->input->post('icf_amount', true),
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['id'])) {
				if ($this->setting_model->create($postData)) {
					...
					addActivityLog("setting", "create", 1, "setting", 1, $postData);
				...
				if ($this->setting_model->update($postData)) {
					...
					addActivityLog("setting", "update", $postData['id'], "setting", 2, $postData);
